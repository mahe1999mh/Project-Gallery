...
const { generateJwt } = require("../middleware/authenticationMiddleware");
const db = require("../config/databaseConfig");

router.post("/signup", (req, res) => {
  const users = req.body;
  db.query(
    "INSERT INTO users ( password, name, email, phoneNumber) VALUES (?, ?, ?, ?)",
    [users.password, users.name, users.email, users.phoneNumber],
    (err, result) => {
      if (err) {
...
router.get("/projects/:projectId", (req, res) => {
  ...
  db.query(
    "SELECT * FROM project WHERE id = ? AND published = 1",
    [id],
    (err, results) => {
      if (err) {
